...
template <bool SimdHash = true, bool First = false>
void transform_left_join_aligned_simd(Dataframe &obj, 
                ...
                const double default_dbl = 0) 
{
    ...
    using map_t = std::conditional_t<
        SimdHash,
        ankerl::unordered_dense::map<std::string_view, MatchGroup, simd_hash>,
        ankerl::unordered_dense::map<std::string_view, MatchGroup>
    >;
    ...
    for (size_t t = 0; t < matr_idx2[2].size(); ++t) {
        ...
        auto* __restrict dst_val = bool_v.data() + nrow * (size_bool + t);
        const auto* __restrict src_val = bool_v2.data() + nrow2 * t;
