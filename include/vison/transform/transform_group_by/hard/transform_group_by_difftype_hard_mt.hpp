...
void transform_group_by_difftype_hard_mt(std::vector<unsigned int>& x,
                                         const n_col int,
                                         ...
                                         F f = &default_groupfn_impl) 
{
    ...
    const unsigned int local_col = ncol;
    ...
    constexpr int key_idx = 0; // dummy key_idx
    ...
    std::vector<uint8_t> key_col = {}; // dummy vec
    ...
    if constexpr (CORES == 1) {
        if constexpr (Function == GroupFunction::Occurence) {
            ...
            dispatch2<Function, 
                      1, // hard mode
                      TColVal,
                      uint8_t, // dummy type (TContainer)
                      OccLookupSameDiffTypeHard,
                      KeyBuildDiffType>(0, 
                                        local_nrow, 
                                        var_lookup,
                                        key_col,
    ...
    dispatch_create_value_col<CreateValueColHard, 
                              Function, 
                              CORES, 
                              RsltTypeKnown,
         /*StandardMethod*/   true>(f, 
                                    var_v_col, 
                                    var_lookup,
                                    local_nrow);
