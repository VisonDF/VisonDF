...
void transform_filter_idx_mt(std::vector<unsigned int>& mask) 
{
    ...
    #pragma omp parallel num_threads(CORES)
    {
      ...
      for (size_t i2 = 0 ; i2 < matr_idx[0].size(); i2 += 1) {
        ...
        const auto col2 = std::vector<std::string>(str_v.data() + nrow * i2, 
                        str_v.data() + nrow * (i2 + 1));
      ...
      for (size_t i2 = 0 ; i2 < matr_idx[1].size(); i2 += 1) {
        ...
        const auto col2 = std::vector<char>(chr_v.data() + nrow * i2, 
                        chr_v.data() + nrow * (i2 + 1));
        for (const auto& pos_idx : mask) {
          val_tmp[nrow_local] = val_tmp2[pos_idx];
          col[nrow_local] = col2[pos_idx];
          nrow_local += 1;
      ...
      for (size_t i2 = 0 ; i2 < matr_idx[2].size(); i2 += 1) {
        ...
        const std::vector<std::string> val_tmp2 = val_tmp;
        unsigned int nrow_local = 0;
        const auto col2 = std::vector<bool>(bool_v.begin() + nrow * i2, 
                        bool_v.begin() + nrow * (i2 + 1));
        for (const auto& pos_idx : mask) {
          val_tmp[nrow_local] = val_tmp2[pos_idx];
          bool_v[nrow2 * i2 + nrow_local] = col2[pos_idx];
          nrow_local += 1;
        }
      };
