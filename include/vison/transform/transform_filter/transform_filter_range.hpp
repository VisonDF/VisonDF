                            ...
                            const unsigned int& strt_vl) 
{
    ...
    for (size_t i2 = 0 ; i2 < matr_idx[0].size(); i2 += 1) {
      ...
      std::vector<std::string>& val_tmp = tmp_val_refv[pos_vl].data();
      nrow_local = 0;
      ...
      for (size_t i = 0; i < mask.size(); ++i) {
        ...
        val_tmp[nrow_local] = std::move(val_tmp[strt_vl + i]);
        col[nrow_local] = col[strt_vl + i];
        nrow_local += 1;
      }
    };
    ...
    for (size_t i2 = 0 ; i2 < matr_idx[1].size(); i2 += 1) {
      ...
      for (size_t i = 0; i < mask.size(); ++i) {
        ...
        val_tmp[nrow_local] = val_tmp[strt_vl + i];
        col[nrow_local] = col[strt_vl + i];
        nrow_local += 1;
      }
    };
    ...
    for (size_t i2 = 0 ; i2 < matr_idx[2].size(); i2 += 1) {
      ...
      for (size_t i = 0; i < mask.size(); ++i) {
        ...
        val_tmp[nrow_local] = val_tmp[strt_vl + i];
        bool_v[nrow * i2 + nrow_local] = bool_v[nrow * i2 + i + strt_vl];
        nrow_local += 1;
      }
    };
