...
inline void radix_sort_int16_mt(const int16_t* keys,
                                size_t* idx,
                                size_t n)
{
    if (n == 0) return;
    ...
    #pragma omp parallel num_threads(THREADS)
    {
        ...
        if constexpr (Simd) {
            histogram_pass_u16_avx512_16buckets(tkeys.data() + beg,
                                               end - beg,
                                               h);
        } else
#endif
        ...
        if constexpr (Simd) {
            if (end - beg < 200'000)
                histogram_pass_u16_avx2(tkeys.data() + beg, end - beg, h);
            else
                histogram_pass_u16_avx2_8buckets(tkeys.data() + beg, end - beg, h);
        } else
#endif
        {
