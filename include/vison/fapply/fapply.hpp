...
void fapply(F f, unsigned int& n) {
    ...
    if constexpr (IsBool) {
        ...
        apply_numeric<MapCol>(bool_v, n, 0, f, local_nrow);

    } else if constexpr (std::is_same_v<T, IntT>) {

        apply_numeric<MapCol>(int_v, n, 3, f, local_nrow);

    } else if constexpr (std::is_same_v<T, UIntT>) {

        apply_numeric<MapCol>(uint_v, n, 4, f, local_nrow);

    } else if constexpr (std::is_same_v<T, FloatT>) {

        apply_numeric<MapCol>(dbl_v, n, 5, f, local_nrow);
