                        ...
                        const std::vector<uint8_t>& mask) 
{
    ...
    if constexpr (IsBool) {
        ...
        apply_numeric_simd_filter<uint8_t>(bool_v, 
                                           n, 
                                           0, 
                                           f, 
                                           mask);
        ...
        apply_numeric_simd_filter<IntT>(int_v, 
                                        n, 
                                        3, 
                                        f, 
                                        mask);
        ...
        apply_numeric_simd_filter<UIntT>(uint_v, 
                                         n, 
                                         4, 
                                         f, 
                                         mask);
        ...
        apply_numeric_simd_filter<FloatT>(dbl_v, 
                                          n, 
                                          5, 
                                          f, 
                                          mask);
        ...
        std::vector<std::string>& val_tmp = tmp_val_refv[n];
        const unsigned int end_val = mask.size();
        for (size_t i = 0; i < start + end_val; ++i) {
            if (!mask[i]) { continue; }
            f(dst[i]);
            val_tmp[i] = dst[i];
        }
