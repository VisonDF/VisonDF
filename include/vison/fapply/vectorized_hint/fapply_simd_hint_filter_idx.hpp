...
void fapply_simd_filter_idx(F f, 
    ...
    if constexpr (IsBool) {
        ...
        apply_numeric_simd_filter_idx<uint8_t>(bool_v, n, 0, f);

    } else if constexpr (std::is_same_v<T, IntT>)
        apply_numeric_simd_filter_idx<IntT>(int_v, n, 3, f);

    else if constexpr (std::is_same_v<T, UIntT>)
        apply_numeric_simd_filter_idx<UIntT>(uint_v, n, 4, f);

    else if constexpr (std::is_same_v<T, FloatT>)
        apply_numeric_simd_filter_idx<FloatT>(dbl_v, n, 5, f);
