...
void display_filter_idx_mt(const std::vector<size_t>& mask,
    ...
    if (name_v_row.size() == 0) {
        ...
        for (size_t i = 0; i < nrow; ++i) {
            const size_t cur_pos = mask[i];
            if (name_v_row[cur_pos].size() > max_nblngth)
                max_nblngth = name_v_row[cur_pos].size();
        };
    };
    ...
    if (name_v_row.size() == 0) {
        for (size_t i = 0; i < nrow; ++i) {
            ...
            std::cout << ":" << cur_pos << ": ";
            std::cout << std::string(max_nblngth - std::to_string(cur_pos).length(), ' ');

            for (size_t& i2 : cols) {
                const std::string& cur_str = tmp_val_refv[i2][cur_pos];
                std::cout << cur_str << " ";
